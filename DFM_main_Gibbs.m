...
Phi_f_true   = 0.99;
sigma_f_true = 1;
Phi_e_true   = diag(rand(N,1)*1.8-0.9);
...
[y, Fact, eps] = DFM_simu_basic(Tob, r, p_f, p_e, Phi_f_true, Phi_e_true, sigma_f_true, sigma_e_true, Loading_true, burn);
save 'y_1.mat',y;
...
i         = T;
z0        = randn(T,1);
Alphas(i) = alpha_11(i) + sqrt(P_11(i)) * z0(i);
    
for i = T-1:-1:1
     alpha_12  = alpha_11(i) + P_11(i)*F'*inv(F*P_11(i)*F'+1)*(Alphas(i+1) - F*alpha_11(i));
     P_12      = P_11(i) - P_11(i)*F'*inv(F*P_11(i)*F'+1)*F*P_11(i);
     Alphas(i) = alpha_12 + sqrt(P_12) * z0(i);
end
