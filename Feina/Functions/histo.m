function histo(TR,str, true)

[T, n]=size(TR);
q=numSubplots(n); % find parameters to optimally arrange subplots

figure; suptitle('Histogram for all parameters'); 

for j=1:n
    subplot(q(1,2),q(1,1),j)
    hist(TR(:,j), 50); title(str(j)) ; xline(true(j), 'y');
end
...
% Rob Campbell function to neatly arrange subplots
% function [p,n]=numSubplots(n)
%
% Purpose
